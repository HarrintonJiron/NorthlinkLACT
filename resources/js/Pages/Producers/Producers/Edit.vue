...
const form = useForm({
  code: props.producer.code || '',
  full_name: props.producer.full_name || '',
  identity_number: props.producer.identity_number || '',
  phone: props.producer.phone || '',
  address: props.producer.address || '',
  community: props.producer.community || '',
  municipality: props.producer.municipality || '',
  department: props.producer.department || '',
  latitude: props.producer.latitude || '',
  longitude: props.producer.longitude || '',
  active: props.producer.active ?? true,
  route_id: props.producer.active_assignment?.route_id || '',
  payment_method: props.producer.active_assignment?.payment_method || 'cash',
})
...
const submit = () => {
  ...
  form.put(`/producers/${props.producer.id}`)
}
</script>
...
<template>
  <AppShell>
    <div class="mb-6">
      ...
      <h1 class="text-3xl font-display font-bold text-[#1D1D1F]">Editar Productor</h1>
      <p class="text-[#6E6E73] mt-1">Actualiza la información del productor</p>
    </div>
    ...
    <form @submit.prevent="submit" class="bg-white rounded-2xl border border-[#E5E5E5] p-6 shadow-sm">
      ...
      <div class="flex items-center justify-end space-x-3 mt-6 pt-4 border-t border-[#E5E5E5]">
        ...
        >
          ...
          {{ form.processing ? 'Guardando...' : 'Actualizar productor' }}
        </button>
      </div>
    </form>
  </AppShell>
